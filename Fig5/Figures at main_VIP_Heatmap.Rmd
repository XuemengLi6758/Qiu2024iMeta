...
```{r load-data}
vip_data <- read.csv("Metabolism_HC_VIPscore.csv",header=T,row.names=1)
...
```{r load-data}
data <- read.csv("metabolism_HC.csv",header=T,row.names=1)
...
```{r plot-scatter}
col_fun <- colorRamp2(c(1, 0, -1),c("#FA7676","#FFFFFF","#749ABF"))
Heatmap(mydata,
        col = col_fun,
        cluster_rows = TRUE,
