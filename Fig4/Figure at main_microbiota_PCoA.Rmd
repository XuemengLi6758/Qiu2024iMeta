...
```{r load-data}
dist_BC <- read.table(as.matrix("Oral_bray_curtis_ASV_file.xls"))
pcoa <- pcoa(dist_BC,correction = "none", rn = NULL) 
groups <- read.table("Oral_CE-group.txt",row.names = 1,header = T,sep = '\t',comment.char = '',check.names = F)
PC1 <- pcoa$vectors[,1]
PC2 <- pcoa$vectors[,2]
...
pcoadata$group <- factor(pcoadata$group,levels = c("CE","NonCE"))
...
res$group <- factor(res$group,levels = c("CE","NonCE"))
```
...
p1 <- ggplot(pcoadata, aes(PC1, PC2,colour = group)) +
  ...
  labs(x = (floor(pcoa$values$Relative_eig[1]*100)) %>% 
       ...
       y = (floor(pcoa$values$Relative_eig[2]*100)) %>% 
         paste0("PCoA2 ( ", ., "%", " )")) +
  scale_colour_manual(values = c("#FA7676","#97D696")) +
...
p2 <- ggplot(pcoadata,aes(group,PC1)) +
  ...
  scale_fill_manual(values = c("#FA7676","#97D696"))+
