...
```{r load-data}
dist_BC <- read.table(as.matrix("Oral_ASV_bray_curtis_PCoA.xls"))
pcoa <- pcoa(dist_BC,correction = "none", rn = NULL) 
groups <- read.table("Oral_CLEY-group.txt",row.names = 1,header = T,sep = '\t',comment.char = '',check.names = F)
PC1 <- pcoa$vectors[,1]
PC2 <- pcoa$vectors[,2]
...
pcoadata$group <- factor(pcoadata$group,levels = c("C","L","E","Y"))
...
res$group <- factor(res$group,levels = c("C","L","E","Y"))
```
